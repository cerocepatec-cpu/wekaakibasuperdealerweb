...
@Component({
  selector: 'app-wekatransfertfounds',
  templateUrl: './wekatransfertfounds.page.html',
  styleUrls: ['./wekatransfertfounds.page.scss'],
    animations: [
...
export class WekatransfertfoundsPage implements OnInit {
...
ChartData:string[]=[];
  ...
  handletransactionchange($event:any,transaction:any){
  ...
  selectalltransactions($event:any){
    if ($event.detail.checked) {
  ...
  updateAccountTotal(){
   ...
   filteraccounts(){
    if (this.accounts && this.accounts.length>0) {
      ...
      if (this.accountsegment==='all') {
        const maxAcc = Math.max(...accoutslist.map((ac:any)=>ac.expenditures_amount+ac.entries_amount));
        let listmax = accoutslist.filter((a:any)=>a.expenditures_amount+a.entries_amount === maxAcc);
        this.maxAccount=listmax;
          
        const minAcc = Math.min(...accoutslist.map((ac:any)=>(ac.expenditures_amount+ac.entries_amount)>0));
        let listmin = accoutslist.filter((a:any)=>a.expenditures_amount+a.entries_amount === minAcc);
        this.minAccount=listmin;
      }
    
      if(this.accountsegment==='entries'){
        const maxAcc = Math.max(...accoutslist.map((ac:any)=>ac.entries_amount));
        let listmax = accoutslist.filter((a:any)=>a.entries_amount === maxAcc);
        this.maxAccount=listmax;
         
        const minAcc = Math.min(...accoutslist.map((ac:any)=>ac.entries_amount));
        let listmin = accoutslist.filter((a:any)=>a.entries_amount === minAcc);
        this.minAccount=listmin;
      }
      
      if(this.accountsegment==='withdraw'){
        const maxAcc = Math.max(...accoutslist.map((ac:any)=>ac.expenditures_amount));
        let listmax = accoutslist.filter((a:any)=>a.expenditures_amount=== maxAcc);
        this.maxAccount=listmax;
        
        const minAcc = Math.min(...accoutslist.map((ac:any)=>ac.expenditures_amount));
        let listmin = accoutslist.filter((a:any)=>a.expenditures_amount === minAcc);
        this.minAccount=listmin;
      } 
    }
   }
