...
import { NewvirtualsaleComponent } from '../newvirtualsale/newvirtualsale.component';
@Component({
  selector: 'app-virtualsales',
  templateUrl: './virtualsales.component.html',
  styleUrls: ['./virtualsales.component.scss'],
})
export class VirtualsalesComponent implements OnInit {
   ...
   @ViewChild('defaultinputsearch') defaultinput!: IonInput;
    ...
    memberlookup() {
      if (this.search.length > 0) {
        ...
        this.Userserv.collectorsAndSuperDealersLookup({
    ...
    async newtransaction(user: Users) {
      const modal = await this.appserv.modalCtrl.create({
        component: NewvirtualsaleComponent,
